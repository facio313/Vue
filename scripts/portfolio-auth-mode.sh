...
if [ -e "$build_contract" ]; then
  [ -f "$build_contract" ] && [ ! -L "$build_contract" ] \
    || fail "$build_contract must be a regular image file"
  {
    ...
    if IFS= read -r build_extra; then
  ...
  [ "$build_branch" = "$resolved_branch" ] && [ "$build_mode" = "$resolved_mode" ] \
...
case "${1:-print}" in
  ...
  exec)
    ...
    ;;
  *)
    fail 'expected print, check, or exec -- <command>'
    ;;
esac
